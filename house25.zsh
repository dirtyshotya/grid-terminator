...
api_url_get_profiles="http://192.168.0.25/api/v1/autotune/presets"
api_url_set_profile="http://192.168.0.25/api/v1/settings"
...
top_profile_file="/home/100acresranch/house/tp25.csv"
bottom_profile_file="/home/100acresranch/house/bp25.csv"
voltage_file="/home/100acresranch/house/voltage.csv"
profiles_json="/home/100acresranch/house/house25profiles.json"
...
while true; do
	...
	if (( $(echo "$voltage < $off_threshold" | bc -l) )); then
		if (( $on == 1 )); then
			curl -L -s -X 'POST' \
				'http://192.168.0.25/api/v1/mining/stop' \
				-H 'accept: */*' \
	...
	if (( $on != 1 )) && [[ $(echo "$voltage > $on_threshold" | bc -l) -eq 1 ]]; then
		curl -X 'POST' \
			'http://192.168.0.25/api/v1/mining/start' \
			-H 'accept: */*' \
			-H 'x-api-key: 100acresranch100acresranch100acr' \
			-d ''
					on=1
